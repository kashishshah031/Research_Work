...
class GraphDecoderOnly(nn.Module):
    def __init__(self, latent_dim, max_num_nodes, node_classes, bond_classes,
                 hidden_dim=512, n_layers=3, dropout=0.1):
        super(GraphDecoderOnly, self).__init__()
        ...
        self.node_head = mlp(latent_dim, max_num_nodes * node_classes)
        self.edge_head = mlp(latent_dim, max_num_nodes * max_num_nodes * bond_classes)
    ...
    def forward(self, H):
        ...
        # Nodes: [B, N, C_node]
        node_logits = self.node_head(H).view(
            B, self.max_num_nodes, self.node_classes
        )

        # Edges: [B, N, N, C_bond]
        edge_logits = self.edge_head(H).view(
            B, self.max_num_nodes, self.max_num_nodes, self.bond_classes
        )

        # Symmetrize over (i,j)
        edge_logits = 0.5 * (edge_logits + edge_logits.transpose(1, 2))

        # Force diagonal to "none" class (assumed index 0) by suppressing others
        idx = torch.arange(self.max_num_nodes, device=edge_logits.device)
        edge_logits[:, idx, idx, 1:] = -1e9
